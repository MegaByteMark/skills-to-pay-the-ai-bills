...
argument-hint: "[prd | fds | full] [create | amend]  # which stream(s) to run, and whether to create fresh or amend an existing doc (mode auto-detected from existing files when omitted)"
---
...
*(This registry is the direct seed for the FDS functional stream and the team's backlog. `Status`: Active | Deprecated.)*
...
*(Note: This structured registry is the direct interface consumed by downstream automated auditing agents. The Source (PRD) column traces each behavior to the product intent that justifies it. `Status`: Active | Deprecated — deprecated rows are retained for history, never deleted.)*
