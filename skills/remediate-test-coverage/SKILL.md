...
4. PHASE 4 (Per-Tier Implementation & Iteration to Green): For the authorised tier only:
     a. Harness Discovery: detect the test runner/framework and existing test layout from manifests and the `tests/*` paths. Do NOT assume a framework. If none exists or the choice is ambiguous, ask exactly ONE `interview-me` question to confirm the runner before scaffolding a harness; never introduce a new framework silently.
     b. Author at the Interface, fake at the Seam: write tests that verify the target Module's Interface (behaviour, invariants, error modes), injecting fake/mock Adapters at its Seams per blueprint §6. Prefer fakes; use mocks only to verify interaction ordering that genuinely matters.
...
- Harness Discovery, Not Assumption: Resolve the runner/framework from evidence; confirm via a single `interview-me` question when ambiguous. Never add a new test dependency or framework without approval.
