...
- Ambiguity Escalation: underspecified/contradictory → (1) `interview-me` for specific gap; (2) if answered, render + note captured interactively; (3) if cannot close → recommend `gather-requirements` `amend`, HALT. When invoked by `po`, report gap to orchestrator.
